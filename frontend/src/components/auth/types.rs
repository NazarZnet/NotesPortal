...
impl From<FormType> for FormSettings {
    fn from(value: FormType) -> Self {
        match value {
            FormType::SignUp => FormSettings { title: "Create an account!".to_owned(), value: "Sign Up".to_owned()},
            FormType::LogIn => FormSettings { title: "Welcome back!".to_owned(), value: "Log In".to_string(), },
        }
    }
}
